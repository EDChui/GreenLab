# Data Exploration Script for RQ1.1
...
# Derive responsible container energy (based on workload type)
data <- data %>%
  mutate(
    responsible_container_energy = case_when(
      tolower(as.character(load_type)) %in% c("media", "media_service") ~ media_service_energy_joules,
      tolower(as.character(load_type)) %in% c("home_timeline", "home-timeline", "home_timeline_service") ~ home_timeline_service_energy_joules,
      tolower(as.character(load_type)) %in% c("compose_post", "compose-post", "compose_post_service") ~ compose_post_service_energy_joules,
      TRUE ~ NA_real_
    )
  )

# RQ1.1 descriptives: responsible container energy by workload and governor
rq1_1_df <- data %>%
  filter(!is.na(responsible_container_energy)) %>%
  select(load_type, load_level, cpu_governor, responsible_container_energy)
...
rq1_1_desc <- calculate_descriptives(
  rq1_1_df, 
  c("load_type", "load_level", "cpu_governor"), 
  "responsible_container_energy"
)

write_csv(rq1_1_desc, file.path(tab_dir, "rq1_1_container_energy_by_workload_governor_descriptives.csv"))

# Governor-level summary table (across all workloads/load levels)
rq1_1_governor_summary <- rq1_1_df %>%
  group_by(cpu_governor) %>%
  summarise(
    n_observations = dplyr::n(),
    Mean = mean(responsible_container_energy, na.rm = TRUE),
    Std = sd(responsible_container_energy, na.rm = TRUE),
    Min = min(responsible_container_energy, na.rm = TRUE),
    `25%` = quantile(responsible_container_energy, 0.25, na.rm = TRUE, names = FALSE),
    Median = median(responsible_container_energy, na.rm = TRUE),
    `75%` = quantile(responsible_container_energy, 0.75, na.rm = TRUE, names = FALSE),
    Max = max(responsible_container_energy, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  arrange(Mean)

write_csv(rq1_1_governor_summary, file.path(tab_dir, "rq1_1_governor_summary_across_all_workloads.csv"))
...
create_boxplot <- function(data, workload_type, fig_dir, governor_colors) {
  ...
  separator_lines <- data.frame(
    ...
    y = min(wt_data$responsible_container_energy, na.rm = TRUE),
    yend = max(wt_data$responsible_container_energy, na.rm = TRUE)
  )
  
  label_data <- data.frame(
    x = c(n_governors/2 + 0.5, n_governors + n_governors/2 + 0.5, 2*n_governors + n_governors/2 + 0.5),
    y = min(wt_data$responsible_container_energy, na.rm = TRUE) * 0.95,
    label = c("High", "Medium", "Low")
  )
  
  p <- ggplot(wt_data, aes(x = x_pos, y = responsible_container_energy, fill = cpu_governor, group = interaction(load_level, cpu_governor))) +
    ...
    labs(x = "", y = "Responsible Container Energy (J)") +
    theme_classic() +
    theme(
  ...
  filename <- paste0("rq1_1_container_energy_boxplot_", tolower(gsub("[^A-Za-z0-9]", "_", workload_type)), ".png")
  ggsave(file.path(fig_dir, filename), p, width = 14, height = 8, dpi = 300)
}

# Create boxplots for each workload type
workload_types <- unique(rq1_1_df$load_type)
governor_colors <- c("#427AB2", "#F09148", "#FF9896", "#DBDB8D", "#C59D94", "#AFC7E8")
names(governor_colors) <- unique(rq1_1_df$cpu_governor)

for (wt in workload_types) {
  create_boxplot(rq1_1_df, wt, fig_dir, governor_colors)
}
...
art_model_container <- art(responsible_container_energy ~ cpu_governor * load_type * load_level, data = data)
anova_result <- anova(art_model_container)
print(anova_result)
...
lm_model_container <- artlm(art_model_container, "cpu_governor")

# Get emmeans for pairwise comparisons
emmeans_result <- emmeans(lm_model_container, ~ cpu_governor)
...
write_csv(pairwise_df, file.path(tab_dir, "rq1_1_posthoc_pairwise_comparisons_holm.csv"))
...
for (i in 1:(length(governors) - 1)) {
  for (j in (i + 1):length(governors)) {
    delta <- calculate_cliffs_delta(data, governors[i], governors[j], "responsible_container_energy")
...
write_csv(cliffs_results, file.path(tab_dir, "rq1_1_cliffs_delta_effect_sizes.csv"))
