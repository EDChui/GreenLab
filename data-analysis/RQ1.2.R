# Data Exploration Script for RQ1.2
...
# RQ1.2 descriptives: total application energy by workload and governor
rq1_2_df <- data %>%
  filter(!is.na(total_application_energy)) %>%
  select(load_type, load_level, cpu_governor, total_application_energy)
...
rq1_2_desc <- calculate_descriptives(
  rq1_2_df, 
  c("load_type", "load_level", "cpu_governor"), 
  "total_application_energy"
)

write_csv(rq1_2_desc, file.path(tab_dir, "rq1_2_application_energy_by_workload_governor_descriptives.csv"))

# Governor-level summary table (across all workloads/load levels)
rq1_2_governor_summary <- rq1_2_df %>%
  group_by(cpu_governor) %>%
  summarise(
    n_observations = dplyr::n(),
    Mean = mean(total_application_energy, na.rm = TRUE),
    Std = sd(total_application_energy, na.rm = TRUE),
    Min = min(total_application_energy, na.rm = TRUE),
    `25%` = quantile(total_application_energy, 0.25, na.rm = TRUE, names = FALSE),
    Median = median(total_application_energy, na.rm = TRUE),
    `75%` = quantile(total_application_energy, 0.75, na.rm = TRUE, names = FALSE),
    Max = max(total_application_energy, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  arrange(Mean)

write_csv(rq1_2_governor_summary, file.path(tab_dir, "rq1_2_governor_summary_across_all_workloads.csv"))
...
create_boxplot <- function(data, workload_type, fig_dir, governor_colors) {
  ...
  separator_lines <- data.frame(
    ...
    y = min(wt_data$total_application_energy, na.rm = TRUE),
    yend = max(wt_data$total_application_energy, na.rm = TRUE)
  )
  
  label_data <- data.frame(
    x = c(n_governors/2 + 0.5, n_governors + n_governors/2 + 0.5, 2*n_governors + n_governors/2 + 0.5),
    y = min(wt_data$total_application_energy, na.rm = TRUE) * 0.95,
    label = c("High", "Medium", "Low")
  )
  
  p <- ggplot(wt_data, aes(x = x_pos, y = total_application_energy, fill = cpu_governor, group = interaction(load_level, cpu_governor))) +
    ...
    labs(x = "", y = "Total Application Energy (J)") +
    theme_classic() +
    theme(
  ...
  filename <- paste0("rq1_2_application_energy_boxplot_", tolower(gsub("[^A-Za-z0-9]", "_", workload_type)), ".png")
  ggsave(file.path(fig_dir, filename), p, width = 14, height = 8, dpi = 300)
}

# Create boxplots for each workload type
workload_types <- unique(rq1_2_df$load_type)
governor_colors <- c("#427AB2", "#F09148", "#FF9896", "#DBDB8D", "#C59D94", "#AFC7E8")
names(governor_colors) <- unique(rq1_2_df$cpu_governor)

for (wt in workload_types) {
  create_boxplot(rq1_2_df, wt, fig_dir, governor_colors)
}

# ART Test for Application-Level Service Energy
...
art_model_application <- art(total_application_energy ~ cpu_governor * load_type * load_level, data = data)
anova_result <- anova(art_model_application)
print(anova_result)
...
# Use ARTool's built-in pairwise comparisons
pairwise_comparisons <- art.con(art_model_application, "cpu_governor", adjust = "holm")
...
write_csv(pairwise_df, file.path(tab_dir, "rq1_2_posthoc_pairwise_comparisons_holm.csv"))
...
for (i in 1:(length(governors) - 1)) {
  for (j in (i + 1):length(governors)) {
    delta <- calculate_cliffs_delta(data, governors[i], governors[j], "total_application_energy")
...
write_csv(cliffs_results, file.path(tab_dir, "rq1_2_cliffs_delta_effect_sizes.csv"))
